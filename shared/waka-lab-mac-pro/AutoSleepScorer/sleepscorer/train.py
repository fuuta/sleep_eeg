...
def create_dataset(config):
    ...
    ind = int(config['test_rate']*x_train.shape[0])-int(config['test_rate']*x_train.shape[0])%config['batch_size']
    x_test = x_train[-ind:]
    y_test = y_train[-ind:]
    x_train = x_train[:-ind]
    y_train = y_train[:-ind]
...
with tf.Graph().as_default():
    ...
    net.dump_confusion_matrix(x_test,y_test,config['batch_size'])
